...
public class SPFacebook : SA_Singleton<SPFacebook> {
	...
	public void LoadUserData() {
		if(IsLoggedIn) {
			...
			FB.API("/me?fields=id,first_name,email", Facebook.HttpMethod.GET, UserDataCallBack);  
			} 
			else 
			{
...
//			Debug.Log(result.Text);
	...
	private void BroadcastLoginResult() {
		if(LoginCallbackResult != null) {
			if(FB.IsLoggedIn) {
				dispatch(FacebookEvents.AUTHENTICATION_SUCCEEDED, LoginCallbackResult);
			} else {
				dispatch(FacebookEvents.AUTHENTICATION_FAILED, LoginCallbackResult);
			}
